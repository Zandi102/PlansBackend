...
class DbOperation
{
    ...
    public function register2($username, $password, $name, $phone, $age, $description, $image)
    {
        try{
            $stmt = $this->conn->query("INSERT INTO User (username, password, name, phone, age, description, image) VALUES ('$username', '$password', '$name', '$phone', '$age', '$description', '$image')");
            $result = $stmt;
        }
        catch (mysqli_sql_exception $e){ 
            $result = false;
    ...
    {
        $stmt = $this->conn->query("SELECT C.compID, C.compName FROM Company as C, Sponsors as S, Plan as P WHERE C.compID = S.compID AND P.plan_id = S.plan_id AND P.date = '$date'");
        $resultarray = array();
        while($row=mysqli_fetch_assoc($stmt)) {
            $resultarray[] = $row;
        }
